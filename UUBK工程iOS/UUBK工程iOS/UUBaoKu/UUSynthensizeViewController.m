...
@interface UUSynthensizeViewController ()

@end

@implementation UUSynthensizeViewController
static int i = 1;
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)getGoodsData{
    ...
    NSDictionary *dict = @{@"KeyWord":self.KeyWord,@"SortName":@"1",@"SortType":self.SortType,@"PageIndex":@"1",@"PageSize":@"10",@"ClassID":self.ClassID};
    if (i == 1) {
        dict = @{@"KeyWord":self.KeyWord,@"SortName":@"1",@"SortType":@"1",@"PageIndex":@"1",@"PageSize":@"10",@"ClassID":self.ClassID};
        ...
        dict = @{@"KeyWord":self.KeyWord,@"SortName":@"1",@"SortType":@"1",@"PageIndex":[NSString stringWithFormat:@"%i",i],@"PageSize":[NSString stringWithFormat:@"%ld",MIN(self.TotalCount-(i-1)*10, 10)],@"ClassID":self.ClassID};
        [NetworkTools postReqeustWithParams:dict UrlString:urlStr successBlock:^(id responseObject) {
