...
- (instancetype)init {
    ...
    if (self) {
        [self awakeFromNib];
        [self addTarget:self action:@selector(selected) forControlEvents:UIControlEventTouchDown];
    }
    return self;
}
...
- (void)selected{
    if (self.selected) {
        if ([self.title isEqualToString:@"价格▼"]) {
            [self setTitle:@"价格▲" forState:UIControlStateNormal];
        ...
        [[NSNotificationCenter defaultCenter]postNotificationName:@"priceChange" object:nil userInfo:@{@"titleText":self.title}];
    }
