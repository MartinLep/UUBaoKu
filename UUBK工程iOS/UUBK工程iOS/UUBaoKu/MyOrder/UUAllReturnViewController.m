...
@interface UUAllReturnViewController ()

@end

@implementation UUAllReturnViewController
static int j = 1;
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)prepareData{
    ...
    NSDictionary *dict;
    if (j==1) {
        dict = @{@"UserId":UserId,@"PageIndex":[NSString stringWithFormat:@"%i",j],@"PageSize":@"10",@"Statu":@"0"};
        ...
        dict = @{@"UserId":UserId,@"PageIndex":[NSString stringWithFormat:@"%i",j],@"PageSize":[NSString stringWithFormat:@"%ld",MIN(self.totalCount-(j-1)*10, 10)],@"Statu":@"0"};
        [NetworkTools postReqeustWithParams:dict UrlString:urlStr successBlock:^(id responseObject) {
            [MBProgressHUD hideHUDForView:self.view animated:YES];
            for (NSDictionary *dict in responseObject[@"data"] [@"List"]) {
                self.model = [[UUReturnGoodsModel alloc]initWithDictionary:dict];
