//
//  UUAllGroupPurchaseViewController.m
//  UUBaoKu
//
...
@interface UUAllGroupPurchaseViewController ()

@end

@implementation UUAllGroupPurchaseViewController
static int i = 1;
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)prepareData{
    ...
    NSDictionary *dict;
    if (i == 1) {
        dict = @{@"UserId":UserId,@"PageIndex":@"1",@"PageSize":@"10",@"Statu":@"1"};
        ...
        dict = @{@"UserId":UserId,@"PageIndex":[NSString stringWithFormat:@"%i",i],@"PageSize":[NSString stringWithFormat:@"%ld",MIN(self.TotalCount-(i-1)*10, 10)],@"Statu":@"1"};
        [NetworkTools postReqeustWithParams:dict UrlString:urlStr successBlock:^(id responseObject) {
            [MBProgressHUD hideHUDForView:self.view animated:YES];
            for (NSDictionary *dict in responseObject[@"data"] [@"List"]) {
                self.model = [[UUGroupModel alloc]initWithDictionary:dict];
