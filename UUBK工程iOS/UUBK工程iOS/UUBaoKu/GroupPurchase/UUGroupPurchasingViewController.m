//
//  UUGroupPurchasingViewController.m
//  UUBaoKu
//
...
@interface UUGroupPurchasingViewController ()

@end

@implementation UUGroupPurchasingViewController
static int i = 1;
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)prepareData{
   ...
   [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    NSDictionary *dict;
    if (i == 1) {
        dict = @{@"UserId":UserId,@"PageIndex":@"1",@"PageSize":@"10",@"Statu":@"2"};
        ...
        dict = @{@"UserId":UserId,@"PageIndex":[NSString stringWithFormat:@"%i",i],@"PageSize":[NSString stringWithFormat:@"%ld",MIN(self.TotalCount-(i-1)*10, 10)],@"Statu":@"2"};
        [NetworkTools postReqeustWithParams:dict UrlString:urlStr successBlock:^(id responseObject) {
            [MBProgressHUD hideHUDForView:self.view animated:YES];
            for (NSDictionary *dict in responseObject[@"data"] [@"List"]) {
                self.model = [[UUGroupModel alloc]initWithDictionary:dict];
