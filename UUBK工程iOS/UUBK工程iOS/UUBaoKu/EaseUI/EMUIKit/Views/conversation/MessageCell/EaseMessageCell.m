...
NSString *const EaseMessageCellIdentifierRecvLink = @"EaseMessageCellRecvLink";
...
NSString *const EaseMessageCellIdentifierSendLink = @"EaseMessageCellSendLink";
...
+ (void)initialize
{
    ...
    cell.leftBubbleMargin = UIEdgeInsetsMake(8, 25, 8, 10);
    cell.rightBubbleMargin = UIEdgeInsetsMake(8, 10, 8, 25);
        ...
        switch (messageType) {
            case EMMessageBodyTypeText:
            {
                if ([model.message.ext[@"link"] boolValue]) {
                    [_bubbleView setupLinkBubbleView];
                    _bubbleView.titleLab.font = _messageTextFont;
                    _bubbleView.titleLab.textColor = _messageTextColor;
                    _bubbleView.linkLab.font = [UIFont systemFontOfSize:13];
                    _bubbleView.linkLab.textColor = [UIColor colorWithRed:236/255.0 green:236/255.0 blue:236/255.0 alpha:1];
                }else{
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                if ([self.model.message.ext[@"link"] boolValue]) {
                    NSLog(@"%@",model.message.ext);
                    _bubbleView.titleLab.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.message.ext[@"content"] textFont:self.messageTextFont];
                    _bubbleView.linkLab.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.message.ext[@"title"] textFont:self.messageTextFont];
                    [_bubbleView.image sd_setImageWithURL:[NSURL URLWithString:model.message.ext[@"img"]] placeholderImage:[UIImage imageNamed:_model.failImageName]];
                }else{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:_model]) {
        ...
        if (_bubbleView) {
            switch (_messageType) {
                ...
                {
                    if ([self.model.message.ext[@"link"] boolValue]) {
                        [_bubbleView updateLinkMargin:_bubbleMargin];
                    }else{
...
- (void)setMessageTextFont:(UIFont *)messageTextFont
{
    ...
    if (_bubbleView.textLabel) {
        ...
        _bubbleView.titleLab.font = messageTextFont;
        _bubbleView.linkLab.font = [UIFont systemFontOfSize:12];
    }
}
...
- (void)setMessageTextColor:(UIColor *)messageTextColor
{
    ...
    if (_bubbleView.textLabel) {
        _bubbleView.textLabel.textColor =_isSender?[UIColor whiteColor]:UUBLACK;
    }else{
        _bubbleView.titleLab.textColor = _isSender?[UIColor whiteColor]:UUBLACK;
        _bubbleView.linkLab.textColor = [UIColor colorWithRed:236/255.0 green:236/255.0 blue:236/255.0 alpha:1];
    }
}
...
+ (NSString *)cellIdentifierWithModel:(id<IMessageModel>)model
{
    ...
    if (model.isSender) {
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
                if ([model.message.ext objectForKey:@"link"]) {
                    cellIdentifier = EaseMessageCellIdentifierSendLink;
                }else{
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
                if ([model.message.ext objectForKey:@"link"]) {
                    cellIdentifier = EaseMessageCellIdentifierRecvLink;
                }else{
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        ...
        {
            if ([model.message.ext[@"link"] boolValue]) {
                NSString *str = [NSString stringWithFormat:@"%@\n%@",model.message.ext[@"title"],model.message.ext[@"content"]];
                NSAttributedString *text = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:str textFont:cell.messageTextFont];
                ...
                height += (rect.size.height > 60 ? 60:rect.size.height) + 10;
            }else{
