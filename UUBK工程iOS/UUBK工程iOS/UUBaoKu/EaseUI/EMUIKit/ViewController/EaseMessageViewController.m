...
- (instancetype)initWithConversationChatter:(NSString *)conversationChatter
                           conversationType:(EMConversationType)conversationType andToUserInfo:(NSDictionary *)toUser
{
    ...
    if (self) {
        ...
        NSLog(@"%@,%@",self.userDict,_conversation.ext);
        if (conversationType != EMConversationTypeGroupChat) {
            _conversation.ext = @{@"fromNickname":toUser[@"NickName"],@"fromAvatar":toUser[@"FaceImg"]};
...
- (void)sendAMessage:(NSNotification *)note{
    _isCollected = 1;
    NSLog(@"%@----------%@",note.userInfo,note.object);
    [self.atTargets removeAllObjects];
    [self sendTextMessage:note.userInfo[@"content"] withExt:note.userInfo];
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(sendAMessage:) name:@"messageSendSuccess" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(addMessage:) name:@"CallEnd" object:nil];
    //Initialization
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"bg_chat_normal"] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"bg_chat_received_normal"] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];
...
- (void)addMessage:(NSNotification *)note{
    EMMessage *message = [EaseSDKHelper sendTextMessage:note.object[@"text"]
                                                     to:self.conversation.conversationId
                                            messageType:[self _messageTypeFromConversationType]
                                             messageExt:self.userDict];
...
- (UUVideoViewController *)videoPicker{
    if (_videoPicker == nil) {
        _videoPicker = [[UUVideoViewController alloc] init];
        _videoPicker.modalPresentationStyle= UIModalPresentationOverFullScreen;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        ...
        if (url == nil) {
            ...
            WBGImageEditor *editor = [[WBGImageEditor alloc] initWithImage:img delegate:self];
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:{
            if ([model.message.ext[@"link"] boolValue]) {
                NSString *urlStr = model.message.ext[@"url"];
                ...
                NSString *type = array2[coun-2];
                NSString *method = array2[coun-1];
                if ([type isEqualToString:@"article"]) {
                    UUZoneDetailViewController *detail = [UUZoneDetailViewController new];
                    detail.articleId = method;
                    [self.navigationController pushViewController:detail animated:YES];
                }else if ([type isEqualToString:@"activity"]){
                    UUactivityMoreDataViewController *activity = [UUactivityMoreDataViewController new];
                    activity.momentId = method.intValue;
                    [self.navigationController pushViewController:activity animated:YES];
                }else if ([type isEqualToString:@"moment"]){
                    UUCircleOfFriendsDetailViewController *moment = [UUCircleOfFriendsDetailViewController new];
                    moment.momentId = method;
                    [self.navigationController pushViewController:moment animated:YES];
                }else{
                    if (![urlStr isKindOfClass:[NSNull class]] ) {
                        UUJoinDetailViewController *goodsDetail = [UUJoinDetailViewController new];
                        
                        if ([method rangeOfString:@"red.html"].location != NSNotFound ) {
                            if (model.message.chatType != EMChatTypeGroupChat) {
                                if (![model.message.from isEqualToString:UserId]) {
                                    goodsDetail.title = @"领取红包";
                                    goodsDetail.goodsUrl = urlStr;
                ...
                NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:regulaStr
                                                                                       options:NSRegularExpressionCaseInsensitive
                                                                                         error:&error];
                NSArray *arrayOfAllMatches = [regex matchesInString:model.text options:0 range:NSMakeRange(0, [model.text length])];
                
                for (NSTextCheckingResult *match in arrayOfAllMatches)
                {
                    NSString* substringForMatch = [model.text substringWithRange:match.range];
                    if (match.range.location != NSNotFound) {
                        UUJoinDetailViewController *goodsDetail = [UUJoinDetailViewController new];
                        goodsDetail.title = @"优物宝库浏览器";
                        goodsDetail.goodsUrl = substringForMatch;
                        [self.navigationController pushViewController:goodsDetail animated:YES];
                    }
                }
            }
...
- (void)moreViewAudioCallAction:(EaseChatBarMoreView *)moreView
{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
    //    [self.navigationController pushViewController:[EMCallViewController new] animated:YES];
...
- (void)moreViewTakeCollectionAction:(EaseChatBarMoreView *)moreView{
    UUCollectMessageViewController *collectVC = [UUCollectMessageViewController new];
    collectVC.isSend = 1;
    collectVC.userId = self.conversation.conversationId;
    [self.navigationController pushViewController:collectVC animated:YES];
}

- (void)moreViewTakeRedPacketAction:(EaseChatBarMoreView *)moreView{
    UURedPacketViewController *redPacked = [UURedPacketViewController new];
    if (self.conversation.type == EMConversationTypeGroupChat) {
        redPacked.isGroup = 1;
        
    }
    redPacked.targetId = self.conversation.conversationId;
    [self.navigationController pushViewController:redPacked animated:YES];
}

- (void)moreViewTakeRecommendAction:(EaseChatBarMoreView *)moreView{
    UUMyFavoritesViewController *favorites = [UUMyFavoritesViewController new];
    favorites.isSend = 1;
    [self.navigationController pushViewController:favorites animated:YES];
}

- (void)moreViewTakeVideoAction:(EaseChatBarMoreView *)moreView{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
...
{
//    if (_isCollected == 0 ) {
//        message.ext = self.userDict;
//    }
//    [message.ext addEntriesFromDictionary:self.userDict];
    if (![message.ext objectForKey:@"fromNickname"]) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:message.ext];
        [dict addEntriesFromDictionary:self.userDict];
        message.ext = dict;
    }
    NSLog(@"%@",message.ext);
    if (self.conversation.type == EMConversationTypeGroupChat){
        message.chatType = EMChatTypeGroupChat;
        if (![message.ext objectForKey:@"groupName"]) {
            NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:message.ext];
            [dict addEntriesFromDictionary:_conversation.ext];
            message.ext = dict;
        }
    }
...
- (void)sendTextMessage:(NSString *)text withExt:(NSDictionary*)ext
{
    EMMessage *message = [EaseSDKHelper sendTextMessage:text
                                          ...
                                          messageType:[self _messageTypeFromConversationType]
                                           messageExt:ext];
    NSLog(@"%@,,%@",self.conversation.conversationId,ext);
