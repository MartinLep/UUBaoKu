...
- (void)getUserInfoWithUserId:(NSString *)userId{
    ...
    [NetworkTools postReqeustWithParams:dict UrlString:urlString successBlock:^(id responseObject) {
        self.remoteNameLabel.text = responseObject[@"data"][@"NickName"];
        [self.iconImg sd_setImageWithURL:[NSURL URLWithString:responseObject[@"data"][@"FaceImg"]] placeholderImage:HolderImage];
...
- (void)_layoutSubviews
{
    ...
    NSLog(@"............%@..............",self.callSession);
    [self getUserInfoWithUserId:self.callSession.remoteName];
...
- (void)timeTimerAction:(id)sender
{
    ...
    if (hour > 0) {
        self.timeLabel.text = [NSString stringWithFormat:@"%.2i:%.2i:%.2i", hour, m, s];
    }
    else if(m > 0){
        self.timeLabel.text = [NSString stringWithFormat:@"%.2i:%.2i", m, s];
    }
    else{
        self.timeLabel.text = [NSString stringWithFormat:@"00:%.2i", s];
    }
}
