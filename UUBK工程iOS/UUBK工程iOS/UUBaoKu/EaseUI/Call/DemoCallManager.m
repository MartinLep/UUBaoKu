...
@property(strong,nonatomic)UUTabBarViewController *mainViewController;
@end
    ...
    @synchronized (_callLock) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.currentController) {
                ...
                [[self getCurrentVC] presentViewController:self.currentController animated:NO completion:nil];
            }
        });
    }
}
             ...
             error:(EMError *)aError
{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"CallEnd" object:@{@"text":responseStr}];
//    [self sendShareMessage:responseStr withExt:nil andUserId:self.userId];
}
...
{
    self.userId = notify.object[@"chatter"];
    if (notify.object) {
            ...
            @synchronized (self.callLock) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (strongSelf.currentController) {
                        [[strongSelf getCurrentVC] presentViewController:self.currentController animated:NO completion:nil];
                    }
                });
            }
...
- (void)hangupCallWithReason:(EMCallEndReason)aReason
{
    ...
    [self _clearCurrentCallViewAndData];
    [self sendShareMessage:[NSString stringWithFormat:@"%d",aReason] withExt:nil andUserId:UserId];
