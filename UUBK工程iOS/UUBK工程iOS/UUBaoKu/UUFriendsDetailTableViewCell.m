//
//  UUFriendsDetailTableViewCell.m
//  UUBaoKu
//
//  Created by dev on 17/4/8.
...
@implementation UUFriendsDetailTableViewCell
{
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        //        CGFloat BtnWithed = (self.width-67-20-34)/2;
        //        _eventDetails = [[UIButton alloc] initWithFrame:CGRectMake(67, self.contentView.height-35, BtnWithed, 33)];
        //        _eventDetails.backgroundColor = [UIColor redColor];
        //
        //        [self addSubview:_eventDetails];
        ...
        _userIntroLbl = [[UILabel alloc] initWithFrame:CGRectMake(20 + TableHeader + 20-15, 5 + TableHeader/2 , screenWidth - 120, TableHeader/2)];
        _userIntroLbl.numberOfLines = 1;
        _userIntroLbl.font = [UIFont fontWithName:@"PingFangSC-Regular" size:15];
        
        _userIntroLbl.textColor = [UIColor colorWithRed:16/255.0 green:16/255.0 blue:16/255.0 alpha:1];
        ...
        _iconImgArray = [NSMutableArray new];
        _nameArray = [NSMutableArray new];
        _timeArray = [NSMutableArray new];
        _replayImgArray = [NSMutableArray new];
        ...
        _linkBackView.backgroundColor = BACKGROUNG_COLOR;
...
#pragma mark -  //头像 昵称 简介
    ...
    NSLog(@"%d",(int)[tempDate.messageBody.posterImgstr  isEqualToString:@""]);
    
    
    NSLog(@"%@",tempDate.messageBody.posterImgstr);
    if (![tempDate.messageBody.posterImgstr  isEqualToString:@""]&&[str isEqual:[NSNull null]] == NO) {
        
        
        [_userHeaderImage sd_setImageWithURL:[NSURL URLWithString:str]];
        
        
        
    }else{
        [_userHeaderImage setImage:[UIImage imageNamed:@"默认头像"]];
...
#pragma mark - // /////////添加说说view
    ...
    WFTextView *textView = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X+47, 15 + TableHeader, screenWidth - 2 * offSet_X-2*47, 0)];
    ...
    textView.frame = CGRectMake(offSet_X+47, 15 + TableHeader, screenWidth - 2 * offSet_X-47, hhhh);
    ...
    //按钮
    __foldBtn.frame = CGRectMake(offSet_X - 10+47, 15 + TableHeader + hhhh + 10 , 50, 20 );
...
#pragma mark - /////// //图片部分
    ...
    for (int  i = 0; i < [ymData.showImageArray count]; i++) {
        ...
        UIImageView *image = [[UIImageView alloc] initWithFrame:CGRectMake(((screenWidth - 240)/4)*(i%3 + 1) + 60*(i%3)+47-12.25, TableHeader + 10 * ((i/3) + 1) + (i/3) *  ShowImage_H + hhhh + kDistance + (ymData.islessLimit?0:30), 80, ShowImage_H)];
...
#pragma mark - /////点赞部分
    ...
    //链接部分
    _linkBackView.frame = CGRectMake(offSet_X+47, TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight , !ymData.messageBody.linkUrl?0: screenWidth - offSet_X * 2-47, !ymData.messageBody.linkUrl?0:33);
    ...
    //活动详情按钮
    _activityBtn.frame = CGRectMake(offSet_X+47, TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight , ymData.messageBody.isActivite==0?0: 127, !ymData.messageBody.isActivite?0:33);
    WFTextView *favourView = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X + 30+47, TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), screenWidth - 2 * offSet_X - 30-47, 0)];
    ...
    favourView.frame = CGRectMake(offSet_X + 30,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54) + balanceHeight + kReplyBtnDistance, screenWidth - offSet_X * 2 - 30, ymData.favourHeight);
    ...
    //点赞图片的位置
    _favourImage.frame = CGRectMake(offSet_X , favourView.frame.origin.y, (ymData.favourHeight == 0)?0:20,(ymData.favourArray.count == 0)?0:20);
...
#pragma mark - ///// //最下方回复部分
    ...
    for (int i = 0; i < _iconImgArray.count; i++) {
        UIImageView *iconView = _iconImgArray[i];
        if (iconView.superview) {
            [iconView removeFromSuperview];
        }
        UILabel *userNameLab = _nameArray[i];
        if (userNameLab.superview) {
            [userNameLab removeFromSuperview];
        }
        UILabel *timeLab = _timeArray[i];
        if (timeLab.superview) {
            [timeLab removeFromSuperview];
        }
    }
    [_iconImgArray removeAllObjects];
    [_nameArray removeAllObjects];
    [_timeArray removeAllObjects];
    for (int i = 0; i <_replayImgArray.count; i++) {
        UIImageView *image = _replayImgArray[i];
        if (image.superview) {
            [image removeFromSuperview];
        }
    }
    [_replayImgArray removeAllObjects];
    ...
    for (int i = 0; i < ymData.replyDataSource.count; i ++ ) {
        ...
        WFTextView *_ilcoreText = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X+65,13+TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance)-47*2+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), screenWidth - offSet_X * 2-65, 0)];
        
        if (i == 0) {
            backView_Y = TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54);
        }
        ...
        UIImageView *userIcon = [[UIImageView alloc]initWithFrame:CGRectMake(offSet_X+28.5,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), 30, 30)];
        [userIcon sd_setImageWithURL:[NSURL URLWithString:body.repliedUserImg] placeholderImage:PLACEHOLDIMAGE];
        [self.contentView addSubview:userIcon];
        
        [_iconImgArray addObject:userIcon];
        if (i == 0) {
            UIImageView *replayImg = [[UIImageView alloc]initWithFrame:CGRectMake(offSet_X+7.5,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), 11.8, 11)];
            replayImg.image = [UIImage imageNamed:@"圈子评论"];
            [self.contentView addSubview:replayImg];
            [_replayImgArray addObject:replayImg];
        }
        UILabel *nameLab = [[UILabel alloc]initWithFrame:CGRectMake(offSet_X+65,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), screenWidth - offSet_X * 2-65-110, 13)];
        nameLab.font = [UIFont fontWithName:@"PingFangSC-Medium" size:12];
        nameLab.textColor = [UIColor colorWithRed:36/255.0 green:80/255.0 blue:132/255.0 alpha:1];
       
        [self.contentView addSubview:nameLab];
        
        nameLab.text = [NSString stringWithFormat:@"%@:",body.replyUserName];
        [_nameArray addObject:nameLab];
        UILabel *timeLab = [[UILabel alloc]initWithFrame:CGRectMake(offSet_X+182,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), screenWidth - offSet_X * 2-182, 13)];
        timeLab.font = [UIFont systemFontOfSize:10];
        timeLab.textColor = UUGREY;
        timeLab.text = body.timeStr;
        [self.contentView addSubview:timeLab];
        [_timeArray addObject:timeLab];
        ...
        _ilcoreText.frame = CGRectMake(offSet_X+65,13+TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance)+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), screenWidth - offSet_X * 2-65, [_ilcoreText getTextHeight]);
        [self.contentView addSubview:_ilcoreText];
        origin_Y += ([_ilcoreText getTextHeight]+13)<35?35:([_ilcoreText getTextHeight]+13)+5;
        
        backView_H += (_ilcoreText.frame.size.height+13)<30?30:(_ilcoreText.frame.size.height+13);
    ...
    if (ymData.replyDataSource.count == 0) {//没回复的时候
        ...
        replyImageView.frame = CGRectMake(offSet_X, favourView.frame.origin.y-3, screenWidth - offSet_X * 2,ymData.favourHeight);
        _likeBtn.frame = CGRectMake(screenWidth - offSet_X - 18 + 2,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance - 24, 18, 16);
        _replyBtn.frame = CGRectMake(screenWidth - offSet_X - 23.7 -24,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance - 24, 18, 16);
        _timeLab.frame = CGRectMake(offSet_X + 47,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance - 24+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), 130, 14);
        _delBtn.frame = CGRectMake(offSet_X + 47+135,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance - 24+(ymData.messageBody.isActivite==0?0:40)+ (!ymData.messageBody.linkUrl?0:54), 30, 14);
        ...
        _timeLab.frame = CGRectMake(offSet_X + 47, replyImageView.frame.origin.y - 24, 130, 14);
        _delBtn.frame = CGRectMake(offSet_X + 47+135, replyImageView.frame.origin.y - 24, 30, 14);;
    }
