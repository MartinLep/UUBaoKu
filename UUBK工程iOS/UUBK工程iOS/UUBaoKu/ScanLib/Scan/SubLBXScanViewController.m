...
- (void)groupDetailRequest {
    ...
    [NetworkTools postReqeustWithParams:dict UrlString:UrlString successBlock:^(id responseObject) {
        if ([KString(responseObject[@"code"]) isEqualToString:@"200"]) {
            ...
            NSLog(@"..........%@.............",userDict);
            NSDictionary *toUser = @{
                                     @"groupName":responseObject[@"data"][@"groupChatName"],
                                     @"groupImgName":@"群头像"
                                     };
            ChatViewController *groupChatVc = [[ChatViewController alloc] initWithConversationChatter:KString(self.groupChatId) conversationType:EMConversationTypeGroupChat andToUserInfo:toUser];
            
            groupChatVc.conversation.ext = @{
                                             @"groupName":responseObject[@"data"][@"groupChatName"],
                                             @"groupImgName":responseObject[@"data"][@"groupChatIcon"]
                                             };
            groupChatVc.userDict = userDict;
            groupChatVc.navigationItem.title = responseObject[@"data"][@"groupChatName"];
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"扫二维码";
...
- (void)showNextVCWithScanResult:(LBXScanResult*)strResult
{
    ...
    if ([strResult.strScanned rangeOfString:@"groupChatId"].location != NSNotFound) {
        self.groupChatId = [strResult.strScanned componentsSeparatedByString:@"="][1];
        ...
        NSLog(@"加入群聊");
    }else if ([strResult.strScanned rangeOfString:@"userId"].location != NSNotFound){
        UUFriendDetailViewController *deatilVC = [[UUFriendDetailViewController alloc] init];
        deatilVC.friendId = [strResult.strScanned componentsSeparatedByString:@"="][1];
        [self.navigationController pushViewController:deatilVC animated:YES];
        NSLog(@"好友详情");
    }else if ([strResult.strScanned rangeOfString:@"http"].location != NSNotFound){
        UUBrowserViewController *browserVC = [UUBrowserViewController new];
        browserVC.urlStr = strResult.strScanned;
